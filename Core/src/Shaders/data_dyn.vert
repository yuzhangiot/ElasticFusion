...
uniform sampler2D cSampler; // 0 real
uniform sampler2D drSampler; // 1 real
uniform sampler2D drfSampler; // 2 real
uniform usampler2D indexSampler; // 3 predict
uniform sampler2D vertConfSampler;
uniform sampler2D colorTimeSampler;
uniform sampler2D normRadSampler; // 6 predict
...
void main()
{
    ...
    vec3 vPosLocal = getVertex(texcoord.xy, x, y, cam, drSampler); // local point position
    vPosition = pose * vec4(vPosLocal, 1); // global point position
    
    //Filtered position ONLY used for normal and radius calculation
    vec3 vPosition_f = getVertex(texcoord.xy, x, y, cam, drfSampler); // local filter point position
    
    //Color for color, obviously
    vColor = textureLod(cSampler, texcoord.xy, 0.0);
    
    vColor.x = encodeColor(vColor.xyz); // rgb
    vColor.y = 0;
    vColor.z = time; // time
    
    //Normal and radius computed with filtered position / depth map transformed to global coords
    vec3 vNormLocal = getNormal(vPosition_f, texcoord.xy, x, y, cam, drfSampler); // local norm
    vNormRad = vec4(mat3(pose) * vNormLocal, getRadius(vPosition_f.z, vNormLocal.z)); // global norm and radius
    ...
    //If this point and its neighout points are actually a valid vertex (i.e. has depth)
       ...
       vPosLocal.z <= maxDepth)
    {
	    ...
	    for(float i = texcoord.x - (scale * indexXStep * windowMultiplier); i < texcoord.x + (scale * indexXStep * windowMultiplier); i += indexXStep) // [-2, 2)
	    {
	        for(float j = texcoord.y - (scale * indexYStep * windowMultiplier); j < texcoord.y + (scale * indexYStep * windowMultiplier); j += indexYStep) // [-2, 2)
	        {
	           ...
	           if(current > 0U)
	           {
                   ...
                   if(abs((vertConf.z * lambda) - (vPosLocal.z * lambda)) < 0.05) // depth diff is little
                   {
                       float dist = length(cross(ray, vertConf.xyz)) / length(ray); // norm diff
                       
                       vec4 normRad = textureLod(normRadSampler, vec2(i, j), 0.0);
                       
                       // depth diff is small and (norm radius is small or direction is right)
