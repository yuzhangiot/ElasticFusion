...
const int DisplayModel::TEXTURE_DIMENSION = 3072;
const int DisplayModel::MAX_VERTICES = DisplayModel::TEXTURE_DIMENSION * DisplayModel::TEXTURE_DIMENSION;
const int DisplayModel::NODE_TEXTURE_DIMENSION = 16384;
const int DisplayModel::MAX_NODES = DisplayModel::NODE_TEXTURE_DIMENSION / 16; //16 floats per node
...
void DisplayModel::initialise(const int nums,
                               ...
                               const std::vector<Eigen::Vector4f>& colors)
{
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(Eigen::Vector4f) * normals.size(), normals.data(), GL_STATIC_DRAW);
    glBindBuffer(GL_ARRAY_BUFFER, 0);
    ...
    initProgram->setUniform(Uniform("cam", cam));
...
void DisplayModel::renderPointCloud(pangolin::OpenGlMatrix mvp,
...
const std::pair<GLuint, GLuint> & DisplayModel::model()
{
    return vbos[target];
}

void DisplayModel::fuse(const Eigen::Matrix4f & pose,
                       const int & time,
                       GPUTexture * rgb,
                       GPUTexture * depthRaw,
...
void DisplayModel::clean(const Eigen::Matrix4f & pose,
                        const int & time,
                        GPUTexture * indexMap,
                        GPUTexture * vertConfMap,
                        GPUTexture * colorTimeMap,
...
unsigned int DisplayModel::lastCount()
{
    return count;
}

Eigen::Vector4f * DisplayModel::downloadMap()
{
    glFinish();
